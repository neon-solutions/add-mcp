...
- write Claude Desktop Windows installs to the MSIX LocalCache config when that file exists or the install is a unique `Claude_*` package with no classic file yet. `%APPDATA%\Claude\claude_desktop_config.json` stays in use when it is the only config file. Two `claude_desktop_config.json` files under different `Claude_*` packages, or two `Claude_*` packages with no config in classic AppData or either package, fail instead of picking a directory.
