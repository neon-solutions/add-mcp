...
GitHub Copilot CLI project installs write `.mcp.json`, or reuse `.github/mcp.json` when that file already exists. Copilot CLI does not read `.vscode/mcp.json` (that path is the VS Code agent). `.mcp.json` is shared with Claude Code: removing a server from either agent removes it for both.
