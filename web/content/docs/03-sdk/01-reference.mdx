...
When `agents` is provided, those agents are included even if not detected (with `detected: false`). A detected agent whose config cannot be read is still returned, with `servers: []` and `error` set to the failure message.
...
interface AgentServers {
  ...
  /** Set when this agent's config could not be read */
  error?: string;
}
...
interface InstalledServer {
  ...
  scope: "local" | "global";
  configPath: string;
  /** Key in the config file this server was read from. `""` means the file root (a Copilot CLI bare name map). */
  configKey: string;
}
```
